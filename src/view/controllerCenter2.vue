  ...
  <div class="ctrl-box">
    ...
    <div class="ctrl-body">
      <el-collapse v-model="openTable">
        <el-collapse-item name="2">
          <template slot="title">
            <b v-text="'已安排' + notFinishedNum + '个'"></b>
          </template>
          <el-table
            ref="singleTable"
        ...
        <el-collapse-item :title="'计划' + notArrangedNum + '个'" name="3">
          <el-row style="margin-bottom:20px;">
            <el-col :span="12">
              <div class="query-box-nota">
                <el-input v-model="workNoQuery" placeholder="请输入工号或姓名或车次号"></el-input>
              </div>
              <el-button type="primary" @click="workNoQueryFuc">查询</el-button>
              <el-button type="primary" @click="reset">重置</el-button>
            </el-col>
          </el-row>
          <el-table
            ref="singleTable"
            :data="selectState ? queryArr : leftList.notArrangedList"
            style="width: 100%"
            ...
            <el-table-column
             ...
             >
              <template slot-scope="scope">
                <div :title="scope.row.taskdicname">{{scope.row.taskdicname}}</div>
              </template>
            </el-table-column>
            <el-table-column
              prop="planType"
              ...
              <template slot-scope="scope">
                 <el-tag
                  :type="scope.row.planType === 1 ? 'success' : 'info'"
                  disable-transitions v-text="scope.row.planType === 1 ? '阶段' : '日'"></el-tag>
              </template>
            </el-table-column>
            <el-table-column
              label="司机"
              width="170">
              <template slot-scope="scope">
                <div :title="`工号：${scope.row.workNo} 姓名：${scope.row.name}`" v-text="scope.row.workNo ? `${scope.row.workNo} (${scope.row.name})` : ''"></div>
              </template>
            </el-table-column>
            <el-table-column
              label="计划入寓"
              width="110">
              <template slot-scope="scope">
                <div :title="scope.row.planlivehousetime">{{scope.row.planlivehousetime | formatDayTime}}</div>
              </template>
            </el-table-column>
            <el-table-column
              label="开行时间"
              width="110">
              <template slot-scope="scope">
                <div :title="scope.row.trainDt">{{scope.row.trainDt | formatDayTime}}</div>
              </template>
            </el-table-column>
            <el-table-column
              property="trainNo"
            ...
            <el-table-column
              ...
              width="110">
              <template slot-scope="scope">
                <div :title="scope.row.remindDt">{{scope.row.remindDt | formatDayTime}}</div>
              </template>
            </el-table-column>
            <el-table-column
              label="操作"
              width="65"> 
              <template slot-scope="scope">
                <el-button type="text" @click="createChange(scope.row)">开始</el-button>
              </template>
            </el-table-column>
				  </el-table>
        </el-collapse-item>
    ...
    <el-dialog
      ...
      <h3 class="public-title"><label for="">开行时间：</label>
         <el-date-picker
          v-model="changeBedValRes.trainDt"
          type="datetime"
          style="width: 32%;"
          placeholder="选择日期时间">
        </el-date-picker>
        <b style="marginLeft: 20px;">{{oldChangeBedValRes.trainDt}}</b>
      </h3>
      <h3 class="public-title"><label for="">叫班时间：</label>
        <el-date-picker
          v-model="changeBedValRes.remindPlanedTime1"
          type="datetime"
          style="width: 32%;"
          placeholder="选择日期时间"
          @blur="updateRemindTime1Watcher">
        </el-date-picker>
        <b style="marginLeft: 20px;">{{ oldChangeBedValRes.remindPlanedTime1}}</b>
      </h3>
    ...
    <div v-if="boxShow">
      <record-show-yun-an :boxShow="boxShow" :recordVal="tempRes" @closeRecordBox="closeRecordBox" :readonly="readonly"></record-show-yun-an>
    </div>
  </div>
</template>
...
<script>
  ...
  import recordShowYunAn from '../components/recordShowYunAn'
...
export default {
  data () {
    return {
      ...
      workNoQuery: '',  // 工号查询
      queryArr: [], //  计划里查询出来的列表
      typeFilterList: [{text: '阶段计划', value: '1'},{text: '日计划', value: '2'}],
      selectState: false  // 需要展示查询后的列表还是原始列表。true为查询出来的列表
    }
  },
  components: {
    bedChoose,
  ...
  computed: {
    ...mapState(['lateSwitch', 'username', 'timerList', 'leftList', 'notArrangedIsTodayList', 'notArrangedIsTomorrowList', 'backTop', 'timeAhead', 'supportingFaceFeature', 'isYunanSwitch', 'taskList']),
    ...mapGetters(['notArrangedNum', 'notFinishedNum', 'notArrangedIsTodayNum', 'notArrangedIsTomorrowNum', 'leng'])
  },
  mounted () {
    this.$nextTick(() => {
      // console.log('我在左侧列表')
      this.$store.commit('loadingChangeFalse')
      if (!this.isYunanSwitch) {
  ...
  methods: {
    ...
    workNoQueryFuc () {
      ...
      let arr = this.leftList.notArrangedList
      for (let i in arr) {
        if ((arr[i].trainNo && arr[i].trainNo.toLowerCase().indexOf(this.workNoQuery.toLowerCase()) > -1) || (arr[i].workNo && arr[i].workNo.toLowerCase().indexOf(this.workNoQuery.toLowerCase()) > -1) || (arr[i].driverName && arr[i].driverName.toLowerCase().indexOf(this.workNoQuery.toLowerCase()) > -1)) {
          this.queryArr.push(arr[i])
        }
      }
      if (this.queryArr.length == 0) {
        this.$message.error('未找到与搜索信息匹配的结果')
      }
    },
    reset () {
      this.queryArr = []
    ...
    changeState (command) {
      ...
      if (command[0] === 1) {
        ...
        this.watchRemindPlanedTime1 = this.oldChangeBedValRes.remindPlanedTime1
    ...
    asynCreateChange (state) { // 异步验证
      ...
      this.axios({
        ...
        if (data.type === 1) {
          ...
          if (bedList[bedIndex] > -1 && !bedList[bedIndex].withMobile) {
